 ...
 */
export default async function GenerateSymbol(length: num, Symbols:str[]): Promise<str> {
...
string using the join() method and returned as the final symbol string. */
  ...
  while (length > 0) {
    let FinalRandomNumForSymbol: num = await GenerateNumberIndex(Symbols); // Generate Random Number Index Between 0 and Array Length - 1
...
async function GenerateNumberIndex(ArraySymbol:str[]): Promise<num> {
  let TempRandomNumBer: num = Math.random() * (ArraySymbol.length-1 - 0) + 0; // Generate Random Number Between 0 and Array Length - 1
