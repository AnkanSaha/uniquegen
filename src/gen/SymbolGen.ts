 ...
 */
export default function GenerateSymbol(length: num): str {
...
hash, dollar sign, etc. */
  // Symbol Array
  const SymbolArray: str[] = ["!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "-", "_", "=", "+", "[", "]", "{", "}", ";", ":", "<", ".", ">", "/", "?", "|", "\\", "~", "`"]; // Symbol Array

/* This line of code is initializing a variable `CountRound` with the value of the `length` parameter
passed to the `GenerateSymbol` function. The variable `CountRound` is used to keep track of the
number of rounds or iterations needed to generate the final symbol string of the specified length. */
  // Number of Rounds
...
string using the join() method and returned as the final symbol string. */
  ...
  while (CountRound > 0) {
    ...
    let FinalRandomNumForSymbol: num = Math.round(TempRandomNumBer); // Round the Number
    FinalID.push(SymbolArray[FinalRandomNumForSymbol]); // Push the Number to Final ID
