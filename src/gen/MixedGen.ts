...
final symbol string of the specified length. */
  ...
  while (length > 0) {
    ...
    let FinalRandomNumForMixed: num = await GenerateNumberIndex(MixedArray); // Generate Random Number Index Between 0 and Array Length - 1
...
async function GenerateNumberIndex(ArrayMixed:str[]): Promise<num> {
  let TempRandomNumBer: num = Math.random() * (ArrayMixed.length-1 - 0) + 0; // Generate Random Number Between 0 and Array Length - 1
